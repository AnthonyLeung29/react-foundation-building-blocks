...
/**
 * Simple component that shows all the foundation icons in use with different sizes
 * 
 * @param {*} props 
 */
const FoundationIcons = props => (
  <div className="container">
    <h1>Foundation Icons contains 283 glyphs:</h1>

    {icons.map(icon => (
      <div key={icon} className="glyph">
        <div className="preview-glyphs">
          {sizes.map(size => (
            <i key={"icon-" + size} className={"step " + icon + " size-" + size}></i>
          ))}
        </div>
        <div className="preview-scale">
          {sizes.map(size => (
            <span key={"label-" + size} className="step">{size}</span>
          ))}
        </div>
        <div className="usage">
          <input className="className" type="text" readOnly="readonly" value={"." + icon} />
        </div>
      </div>
    ))}
  </div>
...
const sizes = [12, 14, 16, 18, 21, 24, 36, 48, 60, 72];
